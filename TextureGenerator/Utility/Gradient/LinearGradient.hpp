	...
	namespace Gradient
	{
		...
		public:
			...
			void GetColor(float position, Color::RGBA * colorOut)
			{
				...
				int indexPosition = (int)(std::upper_bound(_points.begin(), _points.end(), position) - _points.begin());

				// Find the two nearest gradient points so that we can perform linear interpolation on the color.
				size_t indexLeft = Tricible::Clamp<size_t>(indexPosition - 1, 0, _points.size() - 1);
				size_t indexRight = Tricible::Clamp<size_t>(indexPosition, 0, _points.size() - 1);

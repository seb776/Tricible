...
def get_cpp_sources(path):
    ...
    for file in file_list:
        ...
        cleaned_file_name = to_unix_path(path + '/' + file)
        if (lower_filename.endswith(source_exts)):
            files[0].append(cleaned_file_name)
        if (lower_filename.endswith(include_exts)):
            files[1].append(cleaned_file_name)
    return files
    
def absolute_path_to_relative(path, project_folder):
    base_path = to_unix_path(project_folder)
    file_path = to_unix_path(path)
